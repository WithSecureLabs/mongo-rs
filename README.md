...
[![crates.io](https://img.shields.io/crates/v/mongo.svg)](https://crates.io/crates/mongo)
[![Documentation](https://docs.rs/mongo/badge.svg)](https://docs.rs/mongo)

A higher-level wrapper on top of the official bson & mongodb crates.

## Overview

The `mongo` crate is an unofficial higher-level wrapper that aims to make the bson & mongodb crates a little bit more rust-like.
...
```toml
futures = "0.3"
mongo = { version = "0.1", features = ["derive"] }
tokio = { version = "0.2", features = ["full"] }
```

```rust
use futures::stream::StreamExt;
use mongo::{Bson, Mongo};
use mongo::{AsFilter, AsUpdate, Collection, Comparator};
...
```toml
mongo = { version = "0.1", features = ["blocking", "derive"] }
```
...
Well as the derives are basically just fancy ways to convert rust types into BSON, they can be used without the `mongo` clients.
Below is an example of the `mongodb` client but using a `mongo` derived user.
